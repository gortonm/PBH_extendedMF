...
def integral_analytic(f_pbh, m_c, m, epsilon):
    prefactor = f_pbh * (m_star)**(3+epsilon) / (2e-8 * np.sqrt(2*np.pi) * sigma)
    integral = np.sqrt(np.pi/2)*sigma*(-m_c**(epsilon - 3))*np.exp(1/2 * (epsilon - 3)**2 * sigma**2) * erf(((epsilon - 3) * sigma**2 + np.log(m_c) - np.log(m))/(np.sqrt(2) * sigma))
    return prefactor * integral

def constraint_analytic(m_range, f_pbh, m_c, epsilon):
    return integral_analytic(f_pbh, m_c, max(m_range), epsilon) - integral_analytic(f_pbh, m_c, min(m_range), epsilon)
...
if "__main__" == __name__:
    ...
    for m_c in mc_evaporation:
        ...
        f_pbh_evap_analytic.append(1 / constraint_analytic(m_range=m_range, f_pbh=1, m_c=m_c, epsilon=0.4))
    ...
    plt.plot(mc_evaporation, 1e-15 * np.array(f_pbh_evap_analytic), label='Calculated (analytic)')
