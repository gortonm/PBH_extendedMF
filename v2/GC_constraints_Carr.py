    ...
    for exponent_PL_lower in [0, 2, 3, 4]:
        ...
        for j in range(len(Deltas)):
            ...
            for i in range(len(constraints_names_short)):
                ...
                if evolved == False:
                    data_filename_LN = data_folder + "/LN_GC_%s" % constraints_names_short[i] + "_Delta={:.1f}_unevolved.txt".format(Deltas[j])
                    data_filename_SLN = data_folder + "/SLN_GC_%s" % constraints_names_short[i]  + "_Delta={:.1f}_unevolved.txt".format(Deltas[j])
                    data_filename_CC3 = data_folder + "/CC3_GC_%s" % constraints_names_short[i]  + "_Delta={:.1f}_unevolved.txt".format(Deltas[j])
                else:
                    data_filename_LN = data_folder + "/LN_GC_%s" % constraints_names_short[i] + "_Delta={:.1f}.txt".format(Deltas[j])
                    data_filename_SLN = data_folder + "/SLN_GC_%s" % constraints_names_short[i]  + "_Delta={:.1f}.txt".format(Deltas[j])
                    data_filename_CC3 = data_folder + "/CC3_GC_%s" % constraints_names_short[i]  + "_Delta={:.1f}.txt".format(Deltas[j])
...
if "__main__" == __name__:
    ...
    # Power-law exponent to use between 1e11g and 1e15g (1e16g if include_extrap_from_1e16g==True).
        ...
        for with_bkg_subtr in [True]:
            ...
            for exponent_PL_lower in [0, 2, 3, 4]:
                ...
                for j in range(len(Deltas)):                
                    ...
                    # Calculate constraints for the numeric MF
                    for n in [1, 2]:
                        
                        for extrap_numeric_upper in [False, True]:
                                                    
                            if extrap_numeric_lower:
                                numeric_string = "extrap_lower_n={:.0f}_".format(n)
                            else:
                                numeric_string = ""
                            if extrap_numeric_upper:
                                numeric_string += "extrap_upper_n={:.0f}_".format(n)
                                
                            if normalised:
                                numeric_string += "normalised_"
                        
                            if include_extrapolated:
                                data_filename_numeric = data_folder + "/numeric_%s" % numeric_string + "1807.03075_" + prop_string + "_Delta={:.1f}_extrapolated_exp{:.0f}.txt".format(Deltas[j], exponent_PL_lower)
                            else:
                                data_filename_numeric = data_folder + "/numeric_%s" % numeric_string + "1807.03075_" + prop_string + "_Delta={:.1f}_extrapolated_exp{:.0f}.txt".format(Deltas[j], exponent_PL_lower)
    
                            # Numeric MF parameters are the function arguments Delta, extrap_lower, extrap_upper_const, normalised, n, gamma, custom_mp, log_interp
                            params_numeric = [Deltas[j], extrap_numeric_lower, extrap_numeric_upper, normalised, n]                                
                            
                            f_pbh_numeric = constraint_Carr(mp_values_numeric, m_delta_total, f_max_total, mf_numeric, params_numeric, evolved)
                            np.savetxt(data_filename_numeric, [mp_values_numeric, f_pbh_numeric], delimiter="\t")
                ...
                for exponent_PL_lower in [-2, 0]:
                    ...
                    for j in range(len(Deltas)):               
                        ...
                        if include_extrapolated:                     
                            ...
                            data_filename_LN = data_folder + "/LN_2112.15463_Delta={:.1f}".format(Deltas[j]) + "_%s" % bkg_string + ".txt".format(exponent_PL_lower)
                            data_filename_SLN = data_folder + "/SLN_2112.15463_Delta={:.1f}".format(Deltas[j]) + "_%s" % bkg_string + ".txt".format(exponent_PL_lower)
                            data_filename_CC3 = data_folder + "/CC3_2112.15463_Delta={:.1f}".format(Deltas[j]) + "_%s" % bkg_string + ".txt".format(exponent_PL_lower)
...
#%% Constraints from 2108.13256 (21cm).
...
if "__main__" == __name__:
    ...
    # Estimated as 0.228 Gyr at z = 17 using https://www.astro.ucla.edu/~wright/CosmoCalc.html
    t = 0.228 * 1e9 * 365.25 * 86400
    ...
    for exponent_PL_lower in [0, -2, -4]:
        ...
        for j in range(len(Deltas)):               
            ...
            if include_extrapolated:                     
                ...
                data_filename_LN = data_folder + "/LN_2108.13256_21cm_Delta={:.1f}.txt".format(Deltas[j])
                data_filename_SLN = data_folder + "/SLN_2108.13256_21cm_Delta={:.1f}.txt".format(Deltas[j])
                data_filename_CC3 = data_folder + "/CC3_2108.13256_21cm_Delta={:.1f}.txt".format(Deltas[j])
...
#%% Constraints from 2204.05337 (e-ASTROGAM).
    ...
    for exponent_PL_lower in [0, 2, 4]:
        ...
        for j in range(len(Deltas)):                
            if include_extrapolated:                     
                ...
                data_filename_LN = data_folder + "/LN_2204.05337_Delta={:.1f}.txt".format(Deltas[j])
                data_filename_SLN = data_folder + "/SLN_2204.05337_Delta={:.1f}.txt".format(Deltas[j])
                data_filename_CC3 = data_folder + "/CC3_2204.05337_Delta={:.1f}.txt".format(Deltas[j])
...
#%% Constraints from 2102.06714 (AMEGO).
    ...
    for evolved in [True, False]:
        ...
        for j in range(len(Deltas)):                
            if include_extrapolated:                     
                ...
                data_filename_LN = data_folder + "/LN_2102.06714_Delta={:.1f}.txt".format(Deltas[j])
                data_filename_SLN = data_folder + "/SLN_2102.06714_Delta={:.1f}.txt".format(Deltas[j])
                data_filename_CC3 = data_folder + "/CC3_2102.06714_Delta={:.1f}.txt".format(Deltas[j])
        ...
        for exponent_PL_lower in [0, 2, 3, 4]:
            ...
            for j in range(len(Deltas)):                
                if include_extrapolated:                     
                    ...
                    data_filename_LN = data_folder + "/LN_2107.02190_Delta={:.1f}.txt".format(Deltas[j])
                    data_filename_SLN = data_folder + "/SLN_2107.02190_Delta={:.1f}.txt".format(Deltas[j])
                    data_filename_CC3 = data_folder + "/CC3_2107.02190_Delta={:.1f}.txt".format(Deltas[j])
...
if "__main__" == __name__:
    ...
    for log_m_min in log_m_min_values:
        ...
        m_delta_extrapolated = 10**np.arange(log_m_min, np.log10(min(m_delta_values))+0.01, 0.1)
        f_max_extrapolated = min(f_max) * np.power(m_delta_extrapolated / min(m_delta_values), exponent_PL_lower)
