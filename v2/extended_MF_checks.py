...
if "__main__" == __name__:
    ...
    f_pbh = constraint_Carr(mc_values, m_mono_values,
                            f_max_values, LN, [sigma_LN])
...
if "__main__" == __name__:
    ...
    # Exact constraint from extended mass function.
    f_pbh_exact = np.power(-0.5 * (mc_values / m_mono_0) * (1/f_max_0) * np.exp(sigma_LN**2/2) * (erf((sigma_LN**2 - np.log(max(m_mono_values) /
                           mc_values)) / (sigma_LN*np.sqrt(2))) - erf((sigma_LN**2 - np.log(min(m_mono_values)/mc_values)) / (sigma_LN*np.sqrt(2)))), -1)
            ...
            for i in range(len(constraints_names)):
                ...
                for k in range(len(constraints_mono_file)):
                    ...
                    f_PBH_k = constraint_Carr(
                        mc_values, m_mono=m_mono_values, f_max=constraint_energy_bin, mf=LN, params=params_LN)
    ...
    for j in range(len(sigmas_LN)):
        ...
        for i in range(len(constraints_names)):
            ...
            for k in range(len(constraints_mono_file)):
                ...
                constraint_energy_bin_truncated = constraint_energy_bin[
                    m_mono_values_init > m_mono_min]
                f_max_values = constraint_energy_bin_truncated[m_mono_values_truncated < m_mono_max]

                # Calculate constraint on f_PBH from each bin
                f_PBH_k = constraint_Carr(
                    mc_values, m_mono=m_mono_values, f_max=f_max_values, mf=LN, params=params_LN)
                energy_bin_constraints.append(f_PBH_k)
        ...
        ax.set_yscale("log")
        ax.legend(fontsize="small")
        ax.set_title("LN, $\Delta={:.1f}".format(Deltas[j]) + "~(m_\mathrm{min}" + "={:.0e}".format(
...
if "__main__" == __name__:
    ...
    for j in range(len(Deltas)):
        ...
        constraints_names, f_PBH_Isatis_SLN = load_results_Isatis(
            mf_string=fname_base_SLN, modified=True, test_mass_range=True, wide=True)
        constraints_names, f_PBH_Isatis_CC3 = load_results_Isatis(
            mf_string=fname_base_CC3, modified=True, test_mass_range=True, wide=True)
        ...
        for i in range(len(constraints_names)):
            ...
            ax2.plot(
                mp_CC3_values, f_PBH_Isatis_CC3[i], label=constraints_names[i], color=colors_evap[i])
            ...
            for k in range(len(constraints_mono_file)):
                ...
                # Calculate constraint on f_PBH from each bin
                f_PBH_k = constraint_Carr(
                    mc_values, m_mono=m_mono_values, f_max=f_max_values, mf=SLN, params=params_SLN)
                energy_bin_constraints_SLN.append(f_PBH_k)

                f_PBH_k = constraint_Carr(
                    mc_values, m_mono=m_mono_values, f_max=f_max_values, mf=CC3, params=params_CC3)
            ...
            ax1.plot(mp_SLN_values, f_PBH_Carr_SLN, marker="x",
                     linestyle="None", color=colors_evap[i])
            ax2.plot(mp_CC3_values, f_PBH_Carr_CC3, marker="x",
                     linestyle="None", color=colors_evap[i])
...
if "__main__" == __name__:
    ...
    for j in range(len(Deltas)):
        ...
        if j >= 5:
            ...
            # Load monochromatic MF constraints calculated using Isatis, to use the method from 1705.05567.
            # Using each energy bin per instrument individually for the monochromatic MF constraint, then obtaining the tightest constraint from each instrument using envelope().
            constraints_names, f_max_values = load_results_Isatis(
                modified=True)
            ...
            for i in range(len(constraints_names)):
                ...
                for k in range(len(constraints_mono_file)):
                    ...
                    min_constraint = constraint_energy_bin[np.argmax(
                        m_mono_values >= m_star)]
                    constraint_energy_bin_modified = []

                    for l, f_max in enumerate(constraint_energy_bin):
                        if m_mono_values[l] < m_star:
                            constraint_energy_bin_modified.append(
                                min_constraint)
                        else:
                            constraint_energy_bin_modified.append(
                                constraint_energy_bin[l])
                    ...
                    # Calculate constraint on f_PBH from each bin
                    f_PBH_k = constraint_Carr(
                        mc_values, m_mono=m_mono_values, f_max=constraint_energy_bin_modified, mf=SLN, params=params_SLN)
                    energy_bin_constraints_SLN.append(f_PBH_k)

                    f_PBH_k = constraint_Carr(
                        mc_values, m_mono=m_mono_values, f_max=constraint_energy_bin_modified, mf=CC3, params=params_CC3)
                    energy_bin_constraints_CC3.append(f_PBH_k)
            ...
            ax1.set_title("SLN, $\Delta={:.1f}$".format(
                Deltas[j]) + "$, f_\mathrm{max}=\mathrm{const.}~(m < M_*)$", fontsize="small")
            ax2.set_title("CC3, $\Delta={:.1f}$".format(
                Deltas[j]) + "$, f_\mathrm{max}=\mathrm{const.}~(m < M_*)$", fontsize="small")
            fig1.set_tight_layout(True)
            fig2.set_tight_layout(True)
...
if "__main__" == __name__:
    ...
    for j in range(len(Deltas)):
        ...
        if j >= 5:
            ...
            ax2.set_xlabel("$m_p~[\mathrm{g}]$")
            ax1.set_title("SLN, $\Delta={:.1f}$".format(
                Deltas[j]) + ", no constraint at $m < M_*$", fontsize="small")
            ax2.set_title("CC3, $\Delta={:.1f}$".format(
                Deltas[j]) + ", no constraint at $m < M_*$", fontsize="small")
            fig1.set_tight_layout(True)
            fig2.set_tight_layout(True)
            fig1.savefig("./Tests/Figures/SLN_MF_f={:.1e}_mstar={:.1e}g_Delta={:.1f}.png".format(
