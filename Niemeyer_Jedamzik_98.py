...
def integrand(delta, sigma):
    return np.power(delta-delta_c, gamma) * np.exp(-delta**2 / (2*sigma**2)) / (sigma * np.sqrt(2*np.pi))
...
sigmas_integrand = np.logspace(-2, -1.0, 5)
deltas = np.logspace(np.log10(delta_c), -1, 1000)
for sigma in sigmas_integrand:
    plt.figure()
    plt.plot(deltas, integrand(deltas, sigma)/max(integrand(deltas, sigma)), label="${:.1e}$".format(sigma))
...
sigmas = np.logspace(-2, -1, 1000)
plt.figure()
plt.plot(sigmas, omega_PBH_new(sigmas), label="Full expression")
plt.plot(sigmas, omega_PBH_new_approx(sigmas), label="Large $x$ expansion of $\mathrm{erf}(x)$")
plt.plot(sigmas, omega_PBH_new_approx_v2(sigmas), label="Expression from Eq. (7)")
