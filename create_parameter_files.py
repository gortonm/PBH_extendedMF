...
mu_pbh_values = 10**np.arange(np.log10(mu_min), np.log10(mu_max), 0.1)
number_of_sd = 8   # number of standard deviations away from median when calculating minimum and maximum PBH mass
...
for i, mu in enumerate(mu_pbh_values):
    ...
    destination_folder = "GC_LN/sigma={:.1f}/{:.0f}".format(sigma, i)

    filename = BlackHawk_path + "scripts/Isatis/BH_launcher/GC_LN_sigma={:.1f}_{:.0f}.txt".format(sigma, i)
    ...
    np.savetxt(filename, parameters, fmt="%s", delimiter = " = ")
