...
def mu(u):
    ...
    c = 2 * ((u-r)**2 / (r**2 * (u+r))) * (1 + r**2) / np.sqrt(4 + (u-r)**2)
    m = k(u)**2
    return (1/np.pi) * (ellipe(m) * a - ellipk(m) * b + elliptic_pi(n(u), np.pi/2, m) * c )
...
for r in r_values:
    ...
    u_134 = findroot(root_func, a=1e-4, b=10, tolerance=1e-3, n_max=1000)
...
for i, Rs in enumerate(Rs_values):
    ...
    for dL in dL_values:
        ...
        u_134 = findroot3(root_func, a=1e-12, b=2, tolerance=1e-3, n_max=1000)
        u_134_values.append(u_134)
